...
export default function TestSuite() {
      ...
      try {
        unsubscribe = client.subscribe(
          ...
          }`,
          {
            ...
            error: (errors) => {
              addError('subscription', errors[0]?.message || 'Subscription error')
              setSubActive(false)
            },
            complete: () => {
  ...
  const testErrorHandling = async () => {
    ...
    try {
      ...
      addResult('errorHandling', {success: false, message: 'Should have thrown error'})
    } catch (error: any) {
  ...
  return (
    <div style={{ padding: '2rem', fontFamily: 'system-ui', backgroundColor: '#000', minHeight: '100vh', color: '#fff' }}>
      ...
      <div style={{ display: 'grid', gap: '1rem' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ margin: 0, color: '#fff' }}>7. Real-time Subscription (WebSocket)</h3>
            <button
          ...
          {subActive && (
            <div style={{ marginTop: '1rem' }}>
              <strong style={{ color: '#fff' }}>Messages received: {subData.length}</strong>
              <pre style={{
                background: '#0a0a0a',
                padding: '0.5rem',
                borderRadius: '4px',
